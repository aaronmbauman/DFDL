...
* IANA charset namefootnote:[IANA is the Internet Assigned Names Authority. See [IANA\]]
* CCSIDfootnote:[CCSID stands for Coded Character Set ID, a decimal number syntax for a coded character set specifier. [CCSID\]]
...
When parsing, an error can occur when decoding characters from their encoded form into the DFDL Infoset character set, ISO 10646 [ISO10646]. This can occur due to invalid byte sequences, or not enough bytes found to make up the full encoding of a character.
...
The definitions of these substitution characters can be conveniently found for many encodings in the ICU Converter Explorer [ICUConvExp].
